...
class CMB2_Field extends CMB2_Base {
	...
	*************************************************************************************************/
		...
		if ( ( $ninja_type =='ninja_checkbox' ) || ( $ninja_type =='ninja_switch_toggle' ) || ( $ninja_type =='ninja_switch_slide' ) || ( $ninja_type =='ninja_checkbox_icon' ) || ( $ninja_type =='ninja_switch_buttons' ) ) { 

			printf( "<div class=\"form-group %s\" data-fieldtype=\"%s\">\n", $this->row_classes(), $this->type() );

			if ( $ninja_type =='ninja_switch_toggle' ) { 
				echo '<div class="custom-control custom-switch ninja-switch-toggle options-' . $bg_unselected . ' selection-' . $bg_selected . ' options-' . $color_unselected . ' selection-' . $color_selected . '">';
			}
			elseif ( $ninja_type =='ninja_switch_slide' ) { 
				echo '<div class="custom-control custom-switch ninja-switch-slide options-' . $bg_unselected . ' selection-' . $bg_selected . ' options-' . $color_unselected . ' selection-' . $color_selected . '">';
			}
			elseif ( $ninja_type =='ninja_switch_buttons' ) { 
				echo '';
			}
			else {
			...
			if ( $ninja_type =='ninja_switch_buttons' ) { 
				echo '</div>';
			}
		...
		elseif ( ( $ninja_type =='ninja_text' ) || ( $ninja_type =='ninja_tel' ) || ( $ninja_type =='ninja_email' ) || ( $ninja_type =='ninja_url' ) || ( $ninja_type =='ninja_password' ) || ( $ninja_type =='ninja_textarea' ) || ( $ninja_type =='ninja_range' ) || ( $ninja_type =='ninja_radio_buttons' ) || ( $ninja_type =='ninja_radio_icons' ) || ( $ninja_type =='ninja_select' ) || ( $ninja_type =='colorpicker' ) || ( $ninja_type =='ninja_radio' ) || ( $ninja_type =='ninja_file' ) ) {
		...
		******************************************************/
			...
			printf( "<div class=\"form-group cmb-row %s\" data-fieldtype=\"%s\">\n", $this->row_classes(), $this->type() );
	...
	public function label() {
		...
		if ( ( $ninja_type=='ninja_checkbox' ) || ( $ninja_type=='ninja_switch_toggle' ) || ( $ninja_type=='ninja_switch_slide' )  || ( $ninja_type=='ninja_checkbox_icon' ) ) {
			$ninja_checkbox = ' class="custom-control-label" ';
		}

		if ( $ninja_type=='ninja_checkbox_icon' ) {
	    ...
	    if ( ( $ninja_type=='ninja_radio_buttons' ) || ( $ninja_type=='ninja_file' ) ) {
			$d_block = ' class="d-block" ';
		}

		return sprintf( "\n" . '<label%1$s %4$s %6$s for="%2$s">%3$s %5$s</label>' . "\n", $style, $this->id(), $this->args( 'name' ), $ninja_checkbox, $icon_check, $d_block );
	}
	...
	protected function get_default_field_args( $args ) {
		...
		return array(
			...
			'preview_size'      => 'file' === $type || 'ninja_file' === $type ? array( 350, 350 ) : array( 50, 50 ),
