 ...
 * @link      https://cmb2.io
 */
class CMB2_Type_Sidekick_Radio_Icons extends CMB2_Type_Multi_Base {
	...
	public function render() {
		$args = $this->parse_args( $this->type, array(
			'class'   => 'radio-icons-list cmb2-list',
			'options' => $this->concat_items( array(
				'label'  => 'test',
				'method' => 'sidekick_radio_icons',
			) ),
	...
	protected function ul( $a ) {
		return sprintf( '<div class="%s">%s</div>%s', $a['class'], $a['options'], $a['desc'] ); // Change <ul> to <div>. 18-10-03 @CG
	}
